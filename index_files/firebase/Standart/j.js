  ...
  MyallBase.child(USERdef)
    ...
    .then(function (snapshot) {
      ...
      if (CekDATA != undefined) {
        AKK = CekDATA["AKK"];
        let W = UserskataData(CekDATA[AKK]);
        puniLokalno(W);
        Turi_ACT_email(CekDATA["email"], CekDATA["REG"]["veri"]);
        document.getElementById("user_para").innerHTML = MYLINKS_URL + "/<br><span style='font-size: 17px;font-weight: bold;position: relative;z-index: 1;'>" + AKK + "</span>";
      } else {
        ADDni_novUSER(a);
      }
    });
...
function SingUP() {
  if (Par[0] && Par[1] && Par[2]) {
    ...
    let TXTO;
    if (!Par[0]) {
      TXTO = "heyy dude take other username..";
    } else if (!Par[1]) {
      TXTO = "this email is unvaeble for sign up..";
    } else {
