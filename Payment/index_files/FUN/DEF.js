...
EL_["BTN_OPEN_PYMNTO"].onclick = function () {
  ...
  amount.value = null;
  console.log("USERSKI ATRB-- ", A);
};

function updejt_btn_txt(a) {
  let TXT = "Your_Username";
  if (a) {
    TXT = a;
  }
  EL_["COPY_COD_BTN"].style.backgroundColor = "aliceblue";
  EL_["COD_TXT_AREA"].value = BTN_MYLINK["prefix"] + BTN_MYLINK["url"] + TXT + BTN_MYLINK["postfix"];
}

function MDL_cod_geter(a) {
  EL_["MDL_GET_CODE"].style.display = a;
  EL_["INP_new_acc"].value = "";
  updejt_btn_txt(false);
}

function copyToClipboard(cod, BTN) {
...
function Precek(a) {
  let R = null;
  if (a.includes("#")) {
    location.href = "#PRICE_DETALS";
    R = a.slice(35);
    if (R && R != "#PRICE_DETALS") {
      EL_["BTN_OPEN_PYMNTO"].setAttribute("user", R);
    }
  }
}
