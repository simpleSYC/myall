...
function CEKuser() {
  document.getElementById("demo").innerHTML = MYLINKS_URL + "/" + USER;
  MyallBase.child("MYLINKS/" + USER)
    .once("value")
    .then(function (snapshot) {
      let x = document.getElementById("PRO_STATUS");
      let PRO = snapshot.val();

      if (PRO != undefined) {
        if (typeof PRO["PROFIL"]["STATUS"]["AKTIV"] != "boolean") {
          DEL_NVG();
        ...
        DEL_NVG();
        x.innerHTML = "<a>this USERNAME <a/><span>" + USER + "</span><a> still dont exist </a><br><br><a target='_self'    style='font-size: 35px;' href=MYLINKS_URL >Create that acc</a>";
      }
    });
}

function puniLokalno(W) {
  Titliranje(location.href.slice(26));
  let V = W["PROFIL"];
  ...
  if (V["STATUS"]["AKTIV"]) {
    for (i in O_Element) {
      for (q in W[i]) {
        ...
        console.log(l);
        if (A.toString() == "true") {
          O_Element[i][q].children[1].setAttribute("GOling", l);
          if (i == "Activity") {
...
function SHWcontact(a) {
  let TXTETO = a.getAttribute("GOling");
...
function Ekstenzite(a) {
  ...
  for (i in a) {
    if (a[i]) {
      EXtrnzite[i]["O"].style.display = "block";
      EXtrnzite[i].src = EXtrnzite[i]["F"];
    } else {
      EXtrnzite[i]["O"].remove();
    }
  }

  if (a["MYLOCAL"]) {
    EXtrnzite["MYLOCAL"]["O"].addEventListener("click", function () {
      window.open(EXtrnzite["MYLOCAL"]["U"] + USER);
    });
  }

  if (a["CONTACT"]) {
    EXtrnzite["CONTACT"]["O"].addEventListener("click", function () {
